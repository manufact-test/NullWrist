...
void storage_save(const AppState *s) {
  ...
  persist_write_int(KEY_TOTAL_CIGS, (int)s->lifetime_avoided_milli);
  persist_write_int(KEY_TOTAL_MONEY, (int)s->lifetime_saved_cents_milli);
...
void storage_record_relapse(AppState *s, time_t now) {
  ...
  s->lifetime_avoided_milli += ((int64_t)elapsed * s->cigarettes_per_day * 1000) / 86400;
  s->lifetime_saved_cents_milli += ((int64_t)elapsed * s->money_cents_per_day * 1000) / 86400;
  s->attempts++;
  s->last_smoked = now;
  storage_save(s);
}
